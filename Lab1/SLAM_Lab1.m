...
error_z = randn(1,n_visible) .* sigma_z;
...
% H_k = zeros(map.n,max(size(map.hat_x)));
% H_k(measurements.ids_f,1) = -1;
% H_k(measurements.ids_f,measurements.ids_f+1)=1;
% z_k = zeros(map.n,1);
% z_k(measurements.ids_f) = measurements.z_f;
% y_tilde_k = z_k - H_k * map.hat_x';
% S_k = (H_k * map.hat_P*H_k'+measurements.R_f);
% S_k = S_k(measurements.ids_f,measurements.ids_f);
% K_k = map.hat_P*H_k'/S_k;
% map.hat_x = (map.hat_x' + K_k * y_tilde_k)';
% map.hat_P= (eye(size(map.hat_P))-K_k*H_k)*map.hat_P; 
...
map.hat_P(measurements.ids_n+1,measurements.ids_n+1) = map.hat_P(1,1)+ measurements.R_k(measurements.z_pos_n,measurements.z_pos_n);
    ...
    map.n = map.n + max(size(measurements.ids_n));
if config.step_by_step
